...
// Use PostgreSQL .
func (t *BormTable) UsePG() *BormTable {
	t.Cfg.UsePG = true
	return t
}
...
func (t *BormTable) Select(res interface{}, args ...BormItem) (int, error) {
	...
	item.SQL = changePlaceHolder(item.SQL)
	if t.Cfg.Debug {
...
func (t *BormTable) insert(prefix string, objs interface{}, args []BormItem) (int, error) {
	...
	tempSQL := changePlaceHolder(sb.String())

	if t.Cfg.Debug {
		log.Println(tempSQL, stmtArgs)
	}

	res, err := t.DB.ExecContext(t.ctx, tempSQL, stmtArgs...)
	if err != nil {
		return 0, err
	}
